...
function VendorResilienceDeck() {
  ...
  const validateAccessCode = () => {
    ...
    // Get valid codes from localStorage (generated by admin tool)
    const storedCodes = JSON.parse(localStorage.getItem('vrDeckValidCodes') || '[]');
    
    // Check if code exists in valid codes list
    const isValid = storedCodes.some(codeObj => codeObj.code === enteredCode && codeObj.status === 'ACTIVE');
